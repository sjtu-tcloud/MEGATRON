            ...
            u64 gpa, u64 gva, u64 pgsize)
{
    ...
    /*old_pfn = (hash_pt_iova_to_hpa(optimus, iova, NULL) >> PAGE_SHIFT);
    if (old_pfn) {
        //iommu_unmap(domain, gva, pgsize);
        vtp_pt_inval_hwtlb_iova(optimus, iova);
    ...
    return vtp_pt_insert_mapping(optimus->pt, iova, pfn << PAGE_SHIFT, pgsize);
    //return hash_pt_insert_mapping(optimus, iova, pfn << PAGE_SHIFT, pgsize);
}
